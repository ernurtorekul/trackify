...
function details() {
  ...
  useEffect(() => {
    if (id) {
      const selectedSeries = seriesData.find((item) => item.id === id);
      ...
      <div className="relative w-full bg-gray-300 rounded-b-3xl shadow-lg">
        <div className="absolute top-6 left-6 z-10">
          <div className="w-fit h-fit p-2 bg-gray-300 opacity-80 rounded-full transition-all">
            <ChevronLeft size={28} className="cursor-pointer text-gray-800" onClick={() => router.back()}/>
          </div>
        </div>
        <div
      ...
      <div className="mt-6 px-6 py-4 bg-white rounded-2xl shadow-lg space-y-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1">
            <div>
              <strong>Original Name:</strong>{series.title}
            </div>
            <div>
              <strong>Release Date:</strong>{series.releaseDate}
            </div>
            <div>
              <strong>Studio:</strong>{series.studio}
            </div>
            <div>
